...
export async function validateRequest(request) {
  ...
  // Preview-only: skip signature + DB-backed token checks when running the mock preview
  // (development with no database configured). Never active in production.
  if (process.env.NODE_ENV !== "production" && !process.env.MONGODB_URI) {
    return { valid: true, user: decodeTokenUnsafe(token) || { username: "admin", isAdmin: true } }
  }
